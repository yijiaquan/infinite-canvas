...
3. 完成生产资产与导演板
   ...
   - 只获取确实服务脚本的素材。
   - 主人选择白模预演时，在 Clip 与分镜覆盖方案完成后、完整导演板生成前调用 `blender-whitebox-previs`。它复用本机人物模型、骨骼动作、场景与道具代理，在同一 Blender 场景中完成共享调度，并为同一 Clip 渲染一条包含全部覆盖与切点的连续预演；不替代正式导演板或最终 AI 视频。通过 QA 的同 Clip 预演可作为 H3 `<Video 1>` 参考资产，补充运镜、调度和动作节拍；完整分镜导演板仍是规划权威，但不进入 Ref2VA 图片槽。
