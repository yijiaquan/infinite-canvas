...
当前 Ref2VA 的完整导演板作为普通 `<Picture 1>` 进入 H3，控制 Shot 顺序、构图、调度、动作阶段和状态交接；当前完整场景板作为后续普通 `<Picture N>`，只控制地理、固定锚点、材质、主光和多方向关系。两者都不得进入 `picture_1_keyframe`，板式、网格、文字和参考页本身不得渲染进成片。`picture_1_keyframe` 保持断开；其余人物、单状态表情、场景和道具参考按结构化绑定顺序生成提供方标签。只有主人明确选择 I2VA 首帧模式或 FL2VA 首尾帧模式，才使用经过独立 QA 的端点图像。
...
9. Failure Avoidance：只供内部检查当前任务真实风险。静态图片按其提供方实际机制处理；H3 最终提示词由官方 `h3-prompt-writing` 转换，保留官方段落、标签与引用语法，描述正文使用英文，对白、歌词和画面可见文字保留原语言；不新增项目自定义的 negative-prompt 字段或禁词规则。
...
参考槽没有固定填满要求。按当前 H3 Clip 的完整导演故事板、实际出镜角色完整身份板、关键近景所需单状态表情参考、当前完整场景板、确实出现的关键道具板和实际 Speaker 选择最小充分集合；导演板与完整场景板各占一个普通 Picture 槽，但都不占首帧端口。制作拆解已标明高风险，或真实 QA 证明某个构图/状态无法由这些板控制时，才追加一张限定 Shot/连续关系职责的干净彩色关系锚点。跨格/跨 Clip 连接道具只要影响行动、剧情或下一开场，就属于关键道具。未使用槽直接断开或省略，禁止用重复或无关素材填槽。
...
同步对白默认采用“当前说话者在画内”的覆盖：精确台词开始前，说话者必须已经在该 `dialogue_owner_shot` 中清楚可见并可与其 Subject/Speaker 唯一对应，脸部和嘴部在当前景别下足以承载表演，但不强制正脸特写。若当前构图只看见听者、其他角色或空景，优先把台词移到说话者可见的 Shot，或先安排有动机的反打、转场/重构、跟摇或人物入画，再让该角色开口；不得让画面中的非说话者承载画外角色的 `<d>`。双人或多人连续换人发言时，除非同一双人镜已能清楚辨认当前说话者，否则每次换 Speaker 都用明确切镜或构图焦点转移建立新的 owner Shot。

画外对白只作为明确叙事设计使用，例如旁白、电话/广播、门外声源、刻意延迟揭示或真实 J-cut/L-cut。此时必须显式写出 Speaker 不在画面内、声源方位/距离/设备及画内人物的听者反应；进入 H3 最终格式时使用官方句式 `[Speaker] (Sx) says in an off-screen voiceover: <d>[Language]...</d> while the corresponding on-screen character's lips remain completely closed.`。无法说明画外必要性时，改为切到说话者。
...
读取 `references/video-shot-semantic-template.md` 建立内部 Master H3 Clip Contract，再由官方 `h3-prompt-writing` 按当前模式写最终英文结构 Prompt。默认选择实际时长、分镜故事板驱动的同场多 Shot Ref2VA；其他 Base 模式只按明确任务且受当前 Canvas UI JSON 支持时选择。Master Contract 不复制项目控制信息或完整 Bible，也不直接作为 H3 文本。多人、对白、关键交互、连续性、悬疑揭示、冲突/技能攻击时额外读取表演、调度、交接与技能参考页。打斗 Clip 默认采用“高密度连续攻防”而非零散招式列表：先定双方事实、战斗载体、空间路线、优势转换和结尾余势，再将每次攻击/应对/位移/环境反馈分给实际各 Panel。一个 H3 Clip 写：
...
每个 H3 Clip 从其中全部精确 Lines 反查实际 Speakers：只传真实发声者的 current Voice 样本，并让每个 Audio / Subject / Speaker / `<d>` 对应。普通连续发言选择唯一 `dialogue_owner_shot`，完整原文只在该 Shot 的一个 `<d>` 中出现一次；若同一句确实跨越有叙事动机的剪辑，则 owner 扩展为相邻 Shot span，在切点两侧的 `<d>` 使用官方 `<scenetrans>` 连接并明确音频无缝连续，原文不重不漏。视频末尾刻意截断才使用 `<cutoff>`。同一 Speaker 的第二次独立发言必须由新触发、新回应或另一说话者明确切开；多人同时说或唱使用官方复合 ID `(S1,S2)`。没有任何台词的 Clip 不传角色 Voice；画外对白和旁白只传实际说话者的 Voice，不因画面中出现其他角色附带样本。
...
读取 `references/provider-boundaries.md` 后，将语义 brief 交给模型对应 Skill 只做官方格式转换。H3 的段落名、任务类型、关系标记、媒体/Subject/Speaker 标签、`[Shot N] At MM:SS.mmm,` 与 `<d>[Chinese]…</d>` 保持固定兼容格式；所有描述正文使用英文，精确对白、歌词和画面可见文字保留原语言。内部导演卡可继续用中文，但不能复制为最终 H3 的字段式 Prompt：
...
提交前静默检查每个 Beat 的 Shot 或画外载体认领、故事目的、可见变化、真实参考绑定、动作负载、镜头动机、时间和对白、结尾状态、Project/Scene Look 与 Color/Lighting State 继承、参考冲突和官方 provider 语法。小说改编项目还必须逐行对照制作剧本中的“小说对白去向表”：归属当前 Clip 的每句精确台词必须落在唯一 Speaker 及一个普通 owner Shot，或一个使用 `<scenetrans>` 的相邻 owner span；原文不得重叠或遗漏。合并或转移项的剧情功能和承接位置必须兑现；删减项必须已有具体理由。出现未登记原文对白、`待 Series 拆解`、无去向台词，或用概述替代已登记完整台词时，提示词编译失败并退回剧本/拆解修正。若 `Look route = project_look_board`，还必须核对本次正式静态请求只绑定了唯一一张最具体的 current Palette。镜头选择必须有目的、起点、触发、路径和落点；固定机位也必须说明画内变化。若精确台词、动作和停顿无法容纳，按真实编辑理由拆成更多 Shot，不得静默压缩或改写。最终 H3 描述正文使用简洁英文：每个 Shot 直接写人物在哪里、做什么、怎样变化及镜头如何看见；不使用嵌套数字条目，也不重复 Voice 控制说明。尤其 `[Shot 3]` 至 `[Shot N]` 不能用省略号、`same as above`、`同上`或一句结果替代。导演卡字段和完整检查清单只保留在上游。每个 Shot 的声音同时核对环境底声交接、动作同步 Foley/SFX 与音乐相对 Cue。
