...
description: Turn current story, visual, motion, sound, and reference requirements into strong image and H3 Clip semantic briefs and prompts. Use for identity boards, scene and prop images, complete adaptive-panel director boards, targeted edits, and prompt quality review; provider Skills only convert the brief into their required syntax.
---
...
- performance reference：完整人物表演板只服务上游分镜导演板，不作为常规 H3 Picture。H3 的表演默认由完整导演板的动作阶段、身体表演、微表情与强度，以及最终 Prompt 的可见动作链共同控制；只有真实视频 QA 反复证明某个近景表演无法从整板继承时，才允许从 current 表演板提取一张无文字、无边框、无网格的干净单状态参考，限定为当前 Clip/Shot 的表演语言，不控制身份、姿态、构图或其他 Shot；
...
H3 最终提示词遵循“来源媒体”和“可见内容”分离：默认 `<Picture 1>` 是当前 Clip 的完整导演故事板；后续 Picture 是实际出镜人物的完整身份板、当前完整场景板和实际关键道具的完整道具板。它们分别控制镜头计划、人物身份、场景地理和道具形制/State，并在 `subject_definitions` 中映射为对应 `<Subject N>`；后文用同一个 `<Subject N>` 追踪成片里的可见内容。提示词明确所有板都是规划或识别参考，最终输出是完整彩色电影画面，不保留网格、编号、箭头、白底或板式。所有 Subject、Picture、Video、Audio 标签必须指向真实已连接且承担该职责的输入。

参考槽没有固定数量。按当前 H3 Clip 的一张完整分镜故事板、实际出镜角色的完整身份板、当前完整场景板、确实出现的关键道具板和实际 Speaker 选择最小充分集合；故事板占一个 Picture 槽，不把分镜裁图分别上传。制作拆解已标明高风险，或真实 QA 证明某个构图/状态无法由这些板控制时，才追加一张限定 Shot/连续关系职责的干净彩色关系锚点。跨格/跨 Clip 连接道具只要影响行动、剧情或下一开场，就属于关键道具。未使用槽直接断开或省略，禁止用重复或无关素材填槽。
...
把稳定身份和动态表演明确分开：身份/Look 只锁脸、年龄、发型、体型、服装和固定配饰；重复主角先继承其文字“角色表演语言”，每个有人物情绪推进的 Clip 再建立 `开场状态 → 可见触发 → 上升/压制/爆发/回落 → 结尾残留状态` 的表演曲线。逐 Shot 重新设计身体姿态、重心、头部、肩膀、手臂/手、腿、躯干、视线、呼吸、微表情与情绪强度。动作 Panel 优先捕捉 `mid_state`，即动作已经开始但结果尚未完成；不要只写“站起、转头、后退”等终态。需要重复近景或细腻情绪弧时读取 [人物表演板方法](references/character-performance-board-method.md)，按需制作完整表演板供分镜导演板使用；H3 默认继续使用完整身份板和已经吸收表演设计的完整导演板。
...
身份板只负责不可变身份与 Look，不是 Shot 表演权威。主要角色出现重复中性脸、表情僵化或需要连续微表情语言时，读取 [人物表演板方法](references/character-performance-board-method.md) 制作一张独立表演板；它不替代身份板，并只服务分镜导演板。H3 不因面部、全身或表演风险自动新增单视角人物图片；先把所需表演、体量和服装信息修进受影响的完整导演板，再使用完整身份板和整板重拍受影响 Clip。
...
- 对白情绪遵循官方 H3 分层：`<d>` 内只保留真正朗读的精确原文和原文标点；`（逐渐激动）`、`（低声）`、`[愤怒地]` 等语气/表演说明不得混入 `<d>`。在标签外按“起始语气 → 变化触发 → 语速/音量/重音/停顿变化 → 说完后的声音与身体状态”写自然中文，并与眼睑、嘴唇/下颌、呼吸、重心和动作同步；
- 连续动作和表演节拍；
- 逐 Shot 身体/表演：身体姿态与重心、头部、肩膀、手臂/手、腿、躯干、视线、呼吸、表情状态与相对强度；表情与身体必须由同一触发共同变化，不用“惊恐表情”代替表演；
...
- 对跑向、靠近、递交、开门、落座、蹲下操作等路径，写上一末态 → 当前起点 → 可见过程/完成动作 → 当前末态 → 下一开场，并以命名锚点、朝向、行动阶段和道具归属表达；
...
H3 Ref2VA 的 `<Picture 1>` 固定为当前 Clip 的完整分镜导演故事板，控制 Shot 顺序、摄影覆盖、人物调度、动作阶段、空间锚点和 Look/情绪递进。之后依次加入实际出镜人物的完整身份板、当前完整场景板和实际关键道具板；静态生图阶段的 same-View 连续参考和单视角派生图不追加为 H3 Picture。实际 Speaker Voice 或关系锚点按需加入。关系锚点作为后续彩色 Picture 时，在 `subject_definitions`、`retention_analysis` 和实际适用 Shot 中一致声明它只冻结开场/关系几何和状态，不冻结摄影、动作或表演。人物、场景与道具从完整板来源抽象成 `<Subject N>`。换场、复杂状态交接、动作/对白过载或真实 QA 失败时新建下一个 Clip。FL2VA 使用两张目的明确的端点帧；两张端点不得来自上一条成片原始尾帧的自动串接。
