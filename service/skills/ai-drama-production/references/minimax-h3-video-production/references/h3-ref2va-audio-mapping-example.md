# H3 Ref2VA 官方 Audio / Subject / Speaker 实例

以下直接保留主人提供的 MiniMax H3 官方完整实例，用于核对 Ref2VA 六段结构以及 `<Audio 1>` 在定义、摘要、保留分析和实际发声 Shot 中的引用方式。实际项目只复用格式与关系，不照搬咖啡店剧情、人物或时长；当前项目按官方规则使用英文描述正文，对白、歌词和画面可见文字保留原语言。
...
- `<Audio 1>` 在 `subject_definitions` 中绑定 `<Subject 3> (S1)`。
- `summary` 明确说明 `<Audio 1>` 是音色参考。
- `retention_analysis` 使用 `reference`，不在此重复 `(S1)`。
- `<Audio 1>` 在真正使用该音色的 `[Shot 1]` 与 `[Shot 3]` 中自然出现。
- `[Shot 2]` 的 `(S2)` 没有连接 Voice 参考，因此不虚构 `<Audio 2>`。
- 完整可朗读文本只放在 `<d>[Language] ...</d>` 中。
